    ...
    <div className="mx-auto w-full max-w-[1600px] pb-10">
      <header className="sticky top-0 z-30 flex flex-col gap-4 border-b border-border/80 bg-background/95 py-4 backdrop-blur-xl lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="flex items-center gap-2 rounded-xl border border-border bg-white p-1 dark:bg-[#0a241f]">
      ...
      <div className="mt-5 grid gap-5 2xl:grid-cols-[260px_minmax(0,1fr)_280px]">
        ...
        <aside className="space-y-4 2xl:sticky 2xl:top-24 2xl:self-start">
          <div className="rounded-2xl border border-border/80 bg-card/85 p-5 shadow-[0_18px_44px_-34px_oklch(0.3_0.03_50)]"><div className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-primary" /><p className="text-sm font-semibold">Review status</p></div><p className="mt-2 text-xs leading-5 text-muted-foreground">Use this module to record which checks have been performed. A completed item does not replace the supporting evidence or approval required by the relevant workflow.</p><div className="mt-5 rounded-xl border border-border bg-white px-3 py-3 dark:bg-[#0a241f]"><p className="font-mono text-[9px] uppercase tracking-[0.1em] text-muted-foreground">Current list</p><p className="mt-1 text-sm font-semibold">{activeProgress.complete} / {activeProgress.total} complete</p></div><Button variant="outline" onClick={clearProgress} disabled={activeProgress.complete === 0} className="mt-3 h-9 w-full gap-1.5 rounded-xl bg-white text-xs dark:bg-[#0a241f]"><RotateCcw className="h-3.5 w-3.5" />Clear this list</Button></div>
        </aside>
      </div>
    </div>
  );
