    ...
    <div className="mx-auto w-full max-w-[1600px] pb-10">
      ...
      <div className="mt-5 grid gap-5 2xl:grid-cols-[260px_minmax(0,1fr)_280px]">
          ...
          <div className="space-y-2 rounded-2xl border border-border/80 bg-card/70 p-2 shadow-[0_18px_44px_-34px_oklch(0.3_0.03_50)]">
            {checklistDefinitions.map(list => {
              ...
              return <button key={list.id} onClick={() => setActiveId(list.id)} className={`w-full rounded-xl border px-3 py-3 text-left transition ${selected ? "border-primary/35 bg-primary/[0.07] shadow-sm" : "border-transparent hover:border-primary/20 hover:bg-primary/[0.025]"}`}>
                <div className="flex items-start justify-between gap-2"><p className="text-xs font-semibold leading-4">{list.name}</p><span className={`shrink-0 rounded-full px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.08em] ${listState.percentage === 100 ? "bg-emerald-100 text-emerald-700" : "bg-secondary text-secondary-foreground"}`}>{listState.complete}/{listState.total}</span></div>
        ...
        <aside className="space-y-4 2xl:sticky 2xl:top-24 2xl:self-start">
          <div className="rounded-2xl border border-border/80 bg-card/85 p-5 shadow-[0_18px_44px_-34px_oklch(0.3_0.03_50)]"><div className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-primary" /><p className="text-sm font-semibold">Review status</p></div><p className="mt-2 text-xs leading-5 text-muted-foreground">Use this module to record which checks have been performed. A completed item does not replace the supporting evidence or approval required by the relevant workflow.</p><div className="mt-5 rounded-xl border border-border bg-white px-3 py-3"><p className="font-mono text-[9px] uppercase tracking-[0.1em] text-muted-foreground">Current list</p><p className="mt-1 text-sm font-semibold">{activeProgress.complete} / {activeProgress.total} complete</p></div><Button variant="outline" onClick={clearProgress} disabled={activeProgress.complete === 0} className="mt-3 h-9 w-full gap-1.5 rounded-xl bg-white text-xs"><RotateCcw className="h-3.5 w-3.5" />Clear this list</Button></div>
          <div className="rounded-2xl border border-primary/15 bg-primary/[0.035] p-5"><div className="flex items-center gap-2"><SearchCheck className="h-4 w-4 text-primary" /><p className="text-sm font-semibold">Easy to maintain</p></div><p className="mt-2 text-xs leading-5 text-muted-foreground">Each list is defined in the shared checklist catalog. Add a new list, section, or check there and it appears here automatically.</p></div>
        </aside>
      </div>
    </div>
  );
