...
function PreviewSection({ section, selected, onSelect, onDrop, onDragStart }: {
  ...
  if (section.type === "header") {
    return shell(<div className="flex h-16 items-center justify-between px-6"><div className="flex items-center gap-2 font-semibold"><span className="h-5 w-5 rounded-md bg-primary/80" />{section.title}</div><div className="hidden gap-5 text-[10px] text-muted-foreground sm:flex"><span>Overview</span><span>Approach</span><span>Contact</span></div><span className="rounded-md bg-[#18354e] px-3 py-2 text-[10px] text-white">Start here</span></div>);
  }
    ...
    <div className="mx-auto w-full max-w-[1600px] pb-10">
      <header className="sticky top-0 z-30 flex flex-col gap-4 border-b border-border/80 bg-background/95 py-4 backdrop-blur-xl lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0"><p className="font-editorial text-xl leading-none tracking-tight">Wireframe Builder</p><p className="mt-1 font-mono text-[9px] uppercase tracking-[0.13em] text-muted-foreground">Strategist Toolbox / developer handoff</p></div>
        </div>
      ...
      <div className="mt-5 grid gap-5 2xl:grid-cols-[260px_minmax(0,1fr)_300px]">
        ...
        <aside className="2xl:sticky 2xl:top-24 2xl:self-start">
          {selected && selectedDefinition ? <div className="overflow-hidden rounded-2xl border border-border/80 bg-card/85 shadow-[0_18px_44px_-34px_oklch(0.3_0.03_50)]"><div className="border-b border-border/70 px-5 py-4"><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-[9px] uppercase tracking-[0.12em] text-primary">Selected section</p><h2 className="mt-1 text-sm font-semibold">{selectedDefinition.label}</h2></div><span className="rounded-full bg-secondary px-2 py-1 font-mono text-[8px] uppercase tracking-[0.08em] text-secondary-foreground">#{selectedIndex + 1}</span></div></div><div className="space-y-4 p-5"><div><label className="mb-1.5 block font-mono text-[9px] font-medium uppercase tracking-[0.11em] text-muted-foreground">Style option</label><select value={selected.variant} onChange={event => updateSelected({ variant: event.target.value })} className="h-10 w-full rounded-xl border border-border bg-white px-3 text-xs outline-none focus:ring-2 focus:ring-primary/20">{selectedDefinition.variants.map(variant => <option key={variant.id} value={variant.id}>{variant.label}</option>)}</select><p className="mt-1.5 text-[10px] leading-4 text-muted-foreground">{selectedDefinition.variants.find(variant => variant.id === selected.variant)?.description}</p></div><div><label className="mb-1.5 block font-mono text-[9px] font-medium uppercase tracking-[0.11em] text-muted-foreground">Section heading</label><Input value={selected.title} onChange={event => updateSelected({ title: event.target.value })} className="h-10 rounded-xl bg-white text-xs" /></div><div><label className="mb-1.5 flex items-center gap-1.5 font-mono text-[9px] font-medium uppercase tracking-[0.11em] text-muted-foreground"><MessageSquareText className="h-3.5 w-3.5 text-primary" />Strategist / design notes</label><Textarea value={selected.note} onChange={event => updateSelected({ note: event.target.value })} placeholder="Call out requirements, content dependencies, accessibility notes, or visual direction for the developer." className="min-h-[120px] rounded-xl border-border bg-white text-xs leading-5" /></div><div className="grid grid-cols-2 gap-2"><Button variant="outline" onClick={() => moveSelected(-1)} disabled={selectedIndex <= 0} className="h-9 gap-1.5 rounded-xl bg-white text-[11px]"><ArrowUp className="h-3.5 w-3.5" />Move up</Button><Button variant="outline" onClick={() => moveSelected(1)} disabled={selectedIndex >= sections.length - 1} className="h-9 gap-1.5 rounded-xl bg-white text-[11px]"><ArrowDown className="h-3.5 w-3.5" />Move down</Button></div><div className="grid grid-cols-2 gap-2"><Button variant="outline" onClick={duplicateSelected} className="h-9 rounded-xl bg-white text-[11px]">Duplicate</Button><Button variant="outline" onClick={removeSelected} className="h-9 gap-1.5 rounded-xl border-destructive/20 bg-white text-[11px] text-destructive hover:bg-destructive/5 hover:text-destructive"><Trash2 className="h-3.5 w-3.5" />Remove</Button></div></div></div> : <div className="rounded-2xl border border-dashed border-border bg-card/65 p-6 text-center"><Menu className="mx-auto h-5 w-5 text-primary" /><p className="mt-3 text-sm font-medium">Select a section</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Choose a canvas section to set its style and leave handoff notes.</p></div>}
        </aside>
      </div>
    </div>
  );
