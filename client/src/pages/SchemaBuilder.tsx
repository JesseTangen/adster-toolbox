...
function SectionTitle({ index, title, description }: { index: string; title: string; description: string }) {
  return (
    <div className="mb-5 flex gap-3">
      <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-primary/25 bg-primary/5 font-mono text-[10px] text-primary">{index}</span>
      <div>
...
export default function SchemaBuilder() {
  const [draft, setDraft] = useState<UnifiedSchemaDraft>(() => createUnifiedSchemaDraft());
  ...
  const schema = useMemo(() => buildUnifiedSchema(draft), [draft]);
  const validation = useMemo(() => validateUnifiedSchemaDraft(draft), [draft]);
  ...
  const subtypeFields = {
    ...
    professional: isLocalBusiness && (isLocalBusinessType(getEffectiveType(localDraft), "LegalService") || getEffectiveType(localDraft) === "ProfessionalService"),
    store: isLocalBusiness && isLocalBusinessType(getEffectiveType(localDraft), "Store"),
  };
  ...
  const persistDraft = useCallback((draftToPersist: UnifiedSchemaDraft) => {
    if (!draftToPersist.schemaType) return;
  ...
  useEffect(() => {
    if (!sessionLoaded || !hasDraftChanges || !draft.schemaType) return;
  ...
  const updateSchema = (update: Partial<UnifiedSchemaDraft>) => {
    setDraft(current => ({ ...current, ...update }));
    setHasDraftChanges(true);
  };

  const updateLocal = <K extends keyof UnifiedSchemaDraft["localBusiness"]>(field: K, value: UnifiedSchemaDraft["localBusiness"][K]) => {
    setDraft(current => ({ ...current, localBusiness: { ...current.localBusiness, [field]: value } }));
    setHasDraftChanges(true);
  };

  const updateFaq = <K extends keyof UnifiedSchemaDraft["faqPage"]>(field: K, value: UnifiedSchemaDraft["faqPage"][K]) => {
    setDraft(current => ({ ...current, faqPage: { ...current.faqPage, [field]: value } }));
    setHasDraftChanges(true);
  };

  const updateFaqQuestion = (id: string, update: Partial<FaqQuestionDraft>) => {
  ...
  const changeLocalBusinessType = (type: string) => {
    setDraft(current => ({ ...current, localBusiness: { ...current.localBusiness, businessType: type, businessSubtype: type } }));
    setHasDraftChanges(true);
  };

  const createNew = () => {
    if (hasDraftChanges && draft.schemaType) {
      persistDraft(draft);
      toast.success("Current schema saved. New schema created.");
    }
    setDraft(createUnifiedSchemaDraft());
  ...
  const duplicateEntry = (entry: SavedSchema) => {
    const duplicate = cloneUnifiedSchemaDraft(entry.draft);
    persistDraft(duplicate);
    setDraft(duplicate);
    setHasDraftChanges(false);
    setAutoSaveStatus("saved");
    toast.success("Schema duplicate created. Update the page-specific fields.");
    window.scrollTo({ top: 0, behavior: "smooth" });
  };
  ...
  const removeEntry = (id: string) => {
    ...
    toast.success("Schema removed from this session");
  };
    ...
    <div className="mx-auto w-full max-w-[1600px] pb-10">
      <header className="sticky top-0 z-30 flex flex-col gap-4 border-b border-border/80 bg-background/95 py-4 backdrop-blur-xl lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-center gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-primary/10 font-mono text-[11px] font-medium text-primary"><Braces className="h-4.5 w-4.5" /></div>
          <div className="min-w-0"><p className="font-editorial text-xl leading-none tracking-tight">Schema Builder</p><p className="mt-1 truncate font-mono text-[9px] uppercase tracking-[0.13em] text-muted-foreground">Adster Creative Toolbox</p></div>
        </div>
      ...
      <main className="mt-5">
        <div className="grid gap-6 2xl:grid-cols-[250px_minmax(0,1fr)_minmax(390px,0.88fr)]">
          <aside className="lift-in-delayed min-w-0 2xl:sticky 2xl:top-24 2xl:self-start">
            <div className="overflow-hidden rounded-2xl border border-border/80 bg-card/80 shadow-[0_18px_44px_-34px_oklch(0.3_0.03_50)]">
              <div className="flex items-center justify-between border-b border-border/70 px-4 py-3.5">
                <div><p className="text-[13px] font-semibold">Your schemas</p><p className="mt-0.5 text-[11px] text-muted-foreground">Saved for this session</p></div>
              ...
              <div className="max-h-[290px] space-y-1 overflow-auto p-2">
                {entries.length === 0 ? <div className="px-3 py-5 text-center"><FilePlus2 className="mx-auto h-5 w-5 text-muted-foreground/55" /><p className="mt-2 text-[11px] leading-4 text-muted-foreground">Choose a schema type and start typing to save schemas automatically during this session.</p></div> : entries.map(entry => (
                  <div key={entry.id} className={`group flex items-center gap-1 rounded-xl p-1 ${activeEntryId === entry.id ? "bg-primary/[0.08]" : "hover:bg-secondary/65"}`}>
                    <button onClick={() => loadEntry(entry)} className="min-w-0 flex-1 rounded-lg px-2 py-2 text-left outline-none focus-visible:ring-2 focus-visible:ring-ring">
                      <p className="truncate text-[12px] font-medium">{getUnifiedSchemaName(entry.draft)}</p>
                      <p className="mt-0.5 truncate font-mono text-[9px] uppercase tracking-[0.08em] text-muted-foreground">{getSchemaBuilderTypeLabel(entry.draft.schemaType)}</p>
                    </button>
                    <button onClick={() => duplicateEntry(entry)} aria-label="Duplicate saved schema" className="rounded-lg p-2 text-muted-foreground opacity-0 transition hover:bg-primary/10 hover:text-primary focus:opacity-100 focus:outline-none group-hover:opacity-100"><CopyPlus className="h-3.5 w-3.5" /></button>
                    <button onClick={() => removeEntry(entry.id)} aria-label="Delete saved schema" className="rounded-lg p-2 text-muted-foreground opacity-0 transition hover:bg-destructive/10 hover:text-destructive focus:opacity-100 focus:outline-none group-hover:opacity-100"><Trash2 className="h-3.5 w-3.5" /></button>
                  </div>
                ))}
              </div>
              <div className="border-t border-border/70 p-2"><Button variant="outline" onClick={createNew} className="h-9 w-full justify-start gap-2 rounded-xl border-dashed bg-transparent text-[12px] hover:bg-secondary/70"><Plus className="h-3.5 w-3.5" /> New schema</Button></div>
            </div>
          </aside>
          ...
          <section className="min-w-0 space-y-5">
            ...
            {isLocalBusiness ? <>
              <div className="rounded-2xl border border-border/80 bg-card/80 p-5 shadow-[0_18px_44px_-34px_oklch(0.3_0.03_50)] sm:p-6">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="sm:col-span-2"><FieldLabel name="url" /><Input value={localDraft.url} onChange={event => updateLocal("url", event.target.value)} className={fieldClass} placeholder="https://example.com/location" /></div>
                  <div><FieldLabel name="telephone" /><Input value={localDraft.telephone} onChange={event => updateLocal("telephone", event.target.value)} className={fieldClass} placeholder="+1-303-555-0123" /></div>
                  <div><FieldLabel name="email" /><Input value={localDraft.email} onChange={event => updateLocal("email", event.target.value)} className={fieldClass} placeholder="hello@example.com" /></div>
                  <div className="sm:col-span-2"><FieldLabel name="logo" /><Input value={localDraft.logo} onChange={event => updateLocal("logo", event.target.value)} className={fieldClass} placeholder="https://example.com/logo.png" /></div>
                  <div className="sm:col-span-2"><FieldLabel name="sameAs" hint="One URL per line or comma-separated" /><Textarea value={localDraft.sameAs} onChange={event => updateLocal("sameAs", event.target.value)} className={textareaClass} placeholder="https://www.instagram.com/yourbusiness" /></div>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-border/80 bg-card/80 p-5 shadow-[0_18px_44px_-34px_oklch(0.3_0.03_50)] sm:p-6">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="sm:col-span-2"><FieldLabel name="streetAddress" /><Input value={localDraft.streetAddress} onChange={event => updateLocal("streetAddress", event.target.value)} className={fieldClass} placeholder="123 Main Street" /></div>
                  <div><FieldLabel name="addressLocality" /><Input value={localDraft.addressLocality} onChange={event => updateLocal("addressLocality", event.target.value)} className={fieldClass} placeholder="City" /></div>
                  <div><FieldLabel name="addressRegion" /><Input value={localDraft.addressRegion} onChange={event => updateLocal("addressRegion", event.target.value)} className={fieldClass} placeholder="State or region" /></div>
                  <div><FieldLabel name="postalCode" /><Input value={localDraft.postalCode} onChange={event => updateLocal("postalCode", event.target.value)} className={fieldClass} placeholder="Postal code" /></div>
                  <div><FieldLabel name="addressCountry" /><Input value={localDraft.addressCountry} onChange={event => updateLocal("addressCountry", event.target.value)} className={fieldClass} placeholder="US" /></div>
                  <div><FieldLabel name="latitude" /><Input value={localDraft.latitude} onChange={event => updateLocal("latitude", event.target.value)} className={fieldClass} placeholder="39.7392" /></div>
                  <div><FieldLabel name="longitude" /><Input value={localDraft.longitude} onChange={event => updateLocal("longitude", event.target.value)} className={fieldClass} placeholder="-104.9903" /></div>
                  <div className="sm:col-span-2" data-opening-hours-editor>
                    ...
                    <div className="space-y-3 rounded-xl border border-border/80 bg-secondary/[0.22] p-3 dark:bg-[#102b40]/60">
                      <label className="flex cursor-pointer items-center gap-2.5 text-[12px] font-medium"><input aria-label="Open 24 hours" type="checkbox" checked={localDraft.open24Hours} onChange={event => updateLocal("open24Hours", event.target.checked)} className="h-4 w-4 accent-primary" /> Open 24/7</label>
                      {!localDraft.open24Hours ? <>
                        {localDraft.openingHoursRows.map((row, rowIndex) => <div key={row.id} className="rounded-lg border border-border/70 bg-white/85 p-3 dark:bg-[#163950]"><div className="flex items-center justify-between gap-3"><p className="font-mono text-[9px] uppercase tracking-[0.11em] text-muted-foreground">Schedule {rowIndex + 1}</p><button onClick={() => updateLocal("openingHoursRows", localDraft.openingHoursRows.filter(item => item.id !== row.id))} aria-label="Remove opening hours row" className="rounded-md p-1 text-muted-foreground hover:bg-destructive/10 hover:text-destructive"><Trash2 className="h-3.5 w-3.5" /></button></div><div className="mt-2 space-y-3"><div><p className="mb-1.5 font-mono text-[9px] uppercase tracking-[0.1em] text-muted-foreground">Days of week</p><div className="flex flex-wrap gap-1.5">{schemaDays.map(day => <label key={day} className={`cursor-pointer rounded-md border px-2 py-1 text-[10px] transition ${row.dayOfWeek.includes(day) ? "border-primary/45 bg-primary/10 text-primary" : "border-border bg-background text-muted-foreground"}`}><input className="sr-only" type="checkbox" checked={row.dayOfWeek.includes(day)} onChange={event => updateHoursRow(row.id, { dayOfWeek: event.target.checked ? [...row.dayOfWeek, day] : row.dayOfWeek.filter(item => item !== day) })} />{day.slice(0, 3)}</label>)}</div></div><div className="grid grid-cols-2 gap-3"><div><p className="mb-1.5 font-mono text-[9px] uppercase tracking-[0.1em] text-muted-foreground">Opens</p><Input aria-label={`Opens schedule ${rowIndex + 1}`} type="time" value={row.opens} onChange={event => updateHoursRow(row.id, { opens: event.target.value })} className={fieldClass} /></div><div><p className="mb-1.5 font-mono text-[9px] uppercase tracking-[0.1em] text-muted-foreground">Closes</p><Input aria-label={`Closes schedule ${rowIndex + 1}`} type="time" value={row.closes} onChange={event => updateHoursRow(row.id, { closes: event.target.value })} className={fieldClass} /></div></div></div></div>)}
                        <Button type="button" variant="outline" onClick={() => updateLocal("openingHoursRows", [...localDraft.openingHoursRows, createOpeningHoursRow()])} className="h-9 w-full gap-2 rounded-lg border-dashed bg-background text-[11px]"><Plus className="h-3.5 w-3.5" /> Add opening hours</Button>
                  ...
                  <div><FieldLabel name="priceRange" /><Input value={localDraft.priceRange} onChange={event => updateLocal("priceRange", event.target.value)} className={fieldClass} placeholder="$$" /></div>
                </div>
              </div>
            ...
            {isFaqPage ? <div className="rounded-2xl border border-border/80 bg-card/80 p-5 shadow-[0_18px_44px_-34px_oklch(0.3_0.03_50)] sm:p-6">
              ...
              <div className="mb-4 rounded-xl border border-primary/15 bg-primary/[0.035] px-3 py-2.5 text-[11px] leading-4 text-muted-foreground"><span className="font-medium text-foreground">Visible-content reminder.</span> FAQPage markup should represent the same questions and answers users can read on the published page.</div>
          ...
          <aside className="min-w-0 space-y-5 2xl:sticky 2xl:top-24 2xl:self-start">
            <div className="overflow-hidden rounded-2xl border border-[#485a73] bg-[#40516a] text-[#f5fbff] shadow-[0_22px_65px_-34px_rgba(0,92,145,0.6)] dark:border-[#2879a5] dark:bg-[#163950] dark:shadow-[0_22px_65px_-34px_rgba(0,0,0,0.7)]">
              <div className="flex items-start justify-between gap-3 border-b border-white/10 px-5 py-4"><div><div className="flex items-center gap-2"><Globe2 className="h-3.5 w-3.5 text-[#b9eeff]" /><p className="font-mono text-[10px] font-medium uppercase tracking-[0.14em] text-[#b9eeff]">Live JSON-LD</p></div><p className="mt-1 text-[12px] text-white/55">{hasType ? `${draft.schemaType} structured data` : "Choose a schema type to begin"}</p></div><Button onClick={copySchema} variant="outline" className="h-8 gap-1.5 rounded-lg border-white/15 bg-white/[0.06] px-2.5 text-[11px] text-white hover:bg-white/[0.12] hover:text-white"><Copy className="h-3.5 w-3.5" /> Copy</Button></div>
              <div className="code-scroll max-h-[480px] overflow-auto p-5"><pre className="font-mono text-[11px] leading-[1.75] text-[#e7f8ff]"><code>{`<script type="application/ld+json">\n${serializedSchema}\n</script>`}</code></pre></div>
