                  ...
                  <div className="space-y-3 rounded-xl border border-border/80 bg-secondary/[0.22] p-3 dark:bg-[#102b40]/60">
                    ...
                    {!(draft.open24Hours ?? false) ? <>
                      {openingHoursRows.map((row, rowIndex) => <div key={row.id} className="rounded-lg border border-border/70 bg-white/85 p-3 dark:bg-[#163950]">
                        ...
                        <div className="mt-2 space-y-3"><div><p className="mb-1.5 font-mono text-[9px] uppercase tracking-[0.1em] text-muted-foreground">Days of week</p><div className="flex flex-wrap gap-1.5">{schemaDays.map(day => <label key={day} className={`cursor-pointer rounded-md border px-2 py-1 text-[10px] transition ${row.dayOfWeek.includes(day) ? "border-primary/45 bg-primary/10 text-primary" : "border-border bg-background text-muted-foreground"}`}><input className="sr-only" type="checkbox" checked={row.dayOfWeek.includes(day)} onChange={event => updateHoursRow(row.id, { dayOfWeek: event.target.checked ? [...row.dayOfWeek, day] : row.dayOfWeek.filter(item => item !== day) })} />{day.slice(0, 3)}</label>)}</div></div><div className="grid grid-cols-2 gap-3"><div><p className="mb-1.5 font-mono text-[9px] uppercase tracking-[0.1em] text-muted-foreground">Opens</p><Input aria-label={`Opens schedule ${rowIndex + 1}`} type="time" value={row.opens} onChange={event => updateHoursRow(row.id, { opens: event.target.value })} className={fieldClass} /></div><div><p className="mb-1.5 font-mono text-[9px] uppercase tracking-[0.1em] text-muted-foreground">Closes</p><Input aria-label={`Closes schedule ${rowIndex + 1}`} type="time" value={row.closes} onChange={event => updateHoursRow(row.id, { closes: event.target.value })} className={fieldClass} /></div></div></div>
                      </div>)}
