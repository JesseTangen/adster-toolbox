...
describe("LocalBusiness shared header", () => {
  it("uses the shared Toolbox header structure while communicating automatic session saving", () => {
    ...
    expect(source).toContain('data-autosave-status');
    expect(source).toContain('"Saving to session"');
    expect(source).toContain('"Saved to session"');
    expect(source).not.toContain('>Save schema</span>');
